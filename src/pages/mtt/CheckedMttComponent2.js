...
function CheckedMttComponent({item, getChange, name ,index,onClickName,active,}) {
    return (
        <div className={main.main}>
            <div className={`d-flex p-2 mt-1 ${main.district}`}
                 style={{backgroundColor: active ?'#48B1AB':'white', position: 'relative'}}>
                <div style={{
                ...
                <div className={'mx-2'} onClick={()=>onClickName(item,index)} style={{color:!active ?'#000':'white',cursor:'pointer'}}>{item.number ? item.number : ''}{item[name]}</div>
            </div>
        </div>
    );
}
