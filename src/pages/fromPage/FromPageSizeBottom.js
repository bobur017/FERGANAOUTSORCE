    ...
    return (
        <div className={`w-100 d-flex justify-content-center`}>
            ...
            {currentPage + 2 < allPageSize ?
                <button  className={'mx-1 myDarkButtons'}  onClick={() => changesPage(currentPage + 2)}>{currentPage + 3}</button> : null}
            <button className={'mx-1 myDarkButtons'} disabled={currentPage+1 === allPageSize}  onClick={() => changesPage(currentPage + 1)}><IoMdArrowForward size={20}/></button>
        </div>
    );
}
